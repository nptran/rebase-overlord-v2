...
import { ToneLocalizations, localizations, mapWizardStepToConcept } from './locales';
...
export default function GitVisualizerPanel({ 
  ...
  const renderMergeVisual = (w: number, h: number) => {
    ...
    return (
      <svg className="w-full h-full font-mono text-[10px]" viewBox={`0 0 ${w} ${h}`} referrerPolicy="no-referrer">
        ...
        {/* M1 node */}
        <g>
          <circle cx="100" cy="50" r="16" className={`stroke-2 transition-all ${isAncestorHot ? (isLight ? 'stroke-indigo-500 fill-indigo-100 animate-pingScale' : 'stroke-indigo-400 fill-indigo-950/50 animate-pingScale') : (`stroke-emerald-400 ${isLight ? 'fill-emerald-50' : 'fill-slate-950'}`)}`} />
          <circle cx="100" cy="50" r="14" className={`${isLight ? 'fill-white stroke-slate-200' : 'fill-slate-950 stroke-slate-800'}`} />
          <text x="100" y="54" textAnchor="middle" className={`font-bold text-[9px] ${isAncestorHot ? (isLight ? 'fill-indigo-900 font-extrabold' : 'fill-indigo-300') : (isLight ? 'fill-emerald-800' : 'fill-emerald-300')}`}>M1</text>
      ...
      <div className="relative overflow-hidden" style={{ width: `${w}px`, height: `${h}px` }} id="push-stage">
        ...
        <div className="grid grid-cols-2 gap-4 h-full relative p-2">
          ...
          <div className={`rounded-xl border p-3 flex flex-col justify-between ${
            ...
            {isFlight && (
              <div className={`text-[7.5px] font-mono border rounded py-1 text-center animate-pulse ${isLight ? 'bg-indigo-100 text-indigo-850 border-indigo-250 font-bold' : 'bg-indigo-500/10 text-indigo-300 border-indigo-500/20'}`}>
  ...
  if (isCollapsed) {
    return (
      <div id="visualizer-panel-collapsed" className={`rounded-xl p-3.5 flex justify-between items-center transition-all duration-200 ${isLight ? 'bg-white text-slate-800 shadow-sm' : 'bg-[#0f172a] text-slate-300'}`}>
  ...
  return (
    <div id="interactive-git-flow-visualizer" className={`border rounded-xl p-5 shadow-md flex flex-col gap-4 relative overflow-hidden transition-all duration-300 ${
      isLight 
        ? 'bg-white border-indigo-200/60 text-slate-800 shadow-indigo-100/10' 
        : 'bg-[#0f172a] border-indigo-550/25 text-slate-300 shadow-indigo-950/20'
    }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-stretch">
        ...
        <div className={`lg:col-span-8 flex flex-col gap-3 justify-center min-h-[260px] rounded-xl p-4 overflow-hidden relative border ${
          ...
          <GraphControls
            ...
            wizard={wizard}
            isSyncedWithWizard={isSyncedWithWizard}
            setIsSyncedWithWizard={setIsSyncedWithWizard}
                  ...
                  if (showAi && aiLoading) {
                    return (
                      <div className="flex flex-col gap-3.5 py-4 select-none animate-pulse">
                        <div className="flex items-center gap-1.5 font-mono font-bold text-[9.5px] text-indigo-400">
