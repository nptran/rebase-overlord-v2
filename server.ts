...
async function safeGenerateContent(ai: GoogleGenAI, model: string, contents: any, config: any): Promise<any> {
  ...
  for (let i = 0; i < uniqueModels.length; i++) {
    ...
    try {
      ...
      console.warn(`[AI SDK Model Safe Retry] Attempt ${i + 1} with model ${activeModel} failed. Status: ${status}. Message: ${err?.message}`);

      // Fail fast on key authorization or syntax issues
...
Ngoài ra, hãy phân tích câu lệnh của người dùng gõ. Nếu câu lệnh bị lỗi chính tả (ví dụ như "git statsu" thay vì "git status" hoặc "git checkoutt" thay vì "git checkout") hoặc thiếu tham số, hãy sửa đổi nó và đề xuất câu đúng vào danh sách 'suggestedCommands'. Trả về ít nhất 3-4 câu lệnh cụ thể, khả thi nhất làm gợi ý hành động tiếp theo.`;
      ...
      try {
        ...
        console.warn("Fell back or failed to parse Gemini JSON output structure:", responseText, parseErr);
      }
    } catch (geminiErr: any) {
      console.warn("Explain command fallback handled cleanly:", geminiErr?.message);
    }
  }
...
3. 'dr_schema': Nếu isSchemaActive là True, hãy phân tích xem các tệp SQL/Prisma/Drizzle/Schema được chỉnh sửa có cần chạy migration hoặc lệnh CLI đồng bộ DB nào không, đưa ra cảnh báo nguy cơ dữ liệu. Nếu isSchemaActive là False, hãy điền giải thích và biện pháp giảm thiểu là chuỗi rỗng "".`;
      ...
      try {
        ...
        console.warn("Fell back or failed to parse AI resolve JSON output:", responseText, parseErr);
      ...
      console.warn("Doctor diagnostic handled with fallback cleanly:", geminiErr?.message);
    }
  }
...
2. Ghép nối hoặc lựa chọn thông minh 2 đoạn mã trên tạo thành mã nguồn hoàn chỉnh ở thuộc tính "resolvedContent". Đảm bảo mã nguồn logic, tuyệt đối không tự ý sờ mó hoặc thay đổi các đoạn code hoạt động ổn định của hai bên khi không có lý do vững chắc!`;
      ...
      try {
        ...
        console.warn("Fell back or failed to parse block AI resolve output:", responseText, parseErr);
      }
    } catch (geminiErr: any) {
      console.warn("Gemini block API call handled cleanly:", geminiErr?.message);
    }
  }
...
3. Viết mã nguồn hợp nhất và điền vào thuộc tính "resolvedContent". Tuyệt đối không tự ý viết thêm biến mới, đổi tên hoặc sửa code hoạt động bình thường nằm bên ngoài! Tuyệt đối không để sót bất kỳ marker xung đột \`<<<<<<<\`, \`=======\`, \`>>>>>>>\` nào bên trong resolvedContent!`;
      ...
      console.warn("Gemini API call for resolve conflict handled with fallback cleanly:", geminiErr?.message);
    }
  }
...
Provide the diagnosis, tactical strategy, explanation/mitigation details, and complexity assessment.`;
      ...
      const response = await safeGenerateContent(ai, 'gemini-3.5-flash', promptUser, {
        systemInstruction,
        responseMimeType: 'application/json',
        responseSchema: {
          type: Type.OBJECT,
      ...
      try {
        ...
        console.warn("Fell back or failed to parse topological AI doctor response:", responseText, parseErr);
      }
    } catch (err: any) {
      console.warn("AI Doctor dynamic topological call query completed with handled fallback:", err?.message);
    }
  }
...
4. Giữ câu trả lời súc tích, định dạng Markdown đẹp mắt, phân tách các dòng chỉ dẫn rõ ràng.`;
      ...
      console.warn("[AI Diagnostics Chat Handled] Exception:", err?.message || err);
      // Fallback on error
      return res.json({ role: 'model', content: `[AI Handled Capacity issue] ${err?.message}. Fallback reply:\n\n${offlineReply}` });
    }
  }
